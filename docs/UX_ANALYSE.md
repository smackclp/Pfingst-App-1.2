...
| 6 | Dashboard-Aufteilung nach Rolle | **"Absagen & unklare Rückmeldungen" (`DashboardFeedbacks`) sowie die Konflikt-/Offene-Schichten-/Unterbesetzt-Statistikkacheln verschwinden komplett aus dem Helfer-Dashboard** und erscheinen nur noch im neuen Verwaltungsbereich für Bereichsleitung/Lagerleitung (aktuell laut Code für alle sichtbar – konkreter Änderungsbedarf in `DashboardView.tsx`/`DashboardStatsGrid.tsx`/`DashboardFeedbacks.tsx`). Gemeinden-/Teilnehmerzahl bleibt für Helfer sichtbar, aber nur noch als **kleine Randnotiz**, nicht als vollwertige Statistik-Kachel. |
| 7 | Berechtigungsmodell (vereinfacht) | **Helfer dürfen grundsätzlich alles**, außer: Schichten/Schichtgruppen anlegen, Lager/Festival anlegen, Personen bearbeiten – das bleibt Bereichsleitung/Lagerleitung vorbehalten. **Ausnahme:** Im Programmbereich (Talentshow, Bunter Abend) dürfen Helfer ebenfalls voll mitbearbeiten (Beiträge anlegen/ändern), da dort ohnehin oft Helfer die Organisation übernehmen. **Bestellliste bearbeiten** (fremde Bestellungen ändern/Status setzen) bleibt auf die von der Lagerleitung benannte(n) Einkäufer\*innen beschränkt – nutzt das bereits vorhandene Feld `is_buyer` in den Personendaten, keine neue Logik nötig. Ein ursprünglich diskutiertes, dienst-spezifisch delegierbares Berechtigungssystem (mehrere "Dienst-Verantwortliche" mit granularen Sonderrechten) wurde als aktuell **zu komplex verworfen** zugunsten dieses einfacheren Modells. |
| 8 | Admin-Oberfläche für Rollen | **Lücke identifiziert:** Die Zugriffsrolle (Helfer/Bereichsleiter/Lagerleitung) lässt sich bisher nur über die API setzen, nicht über die Oberfläche. Lösung: Im bestehenden Personen-Formular (`PersonFormModal.tsx`) wird ein Dropdown "Zugriffsrolle" ergänzt, direkt neben dem bereits vorhandenen "Einkäufer\*in"-Häkchen (`is_buyer`) – kein neuer Screen nötig, nur eine Erweiterung des bestehenden Formulars, sichtbar/bearbeitbar nur für Lagerleitung. Bis diese UI existiert, wurde ein fester Notfall-Admin-Zugang (`user-admin-lagerleitung`, Rolle Lagerleitung) direkt in `server/seed.ts` ergänzt sowie einmalig manuell in die produktive Firestore-Datenbank eingetragen (PIN separat mitgeteilt, sollte nach UI-Fertigstellung geändert werden). |
...
| # | Entscheidung | Status |
|---|---|---|
| 1 | Helles Theme als Standard | ✅ Umgesetzt |
| 2 | Bottom-Navigation | ✅ Umgesetzt |
| 3 | Kalender & Druckansicht zusammenlegen | ✅ Umgesetzt |
| 4 | Schichten-Standardansicht rollenabhängig | ✅ Umgesetzt |
| 5 | Farbpalette | ✅ Umgesetzt |
