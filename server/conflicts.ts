        ...
        if (s1.date === s2.date) {
          ...
          if (start1 < end2 && start2 < end1) {
            ...
            conflicts.push({
              ...
              type: "overlap",
              message: `Überschneidung am ${getDayName(s1.date)}, ${formatDateGerman(s1.date)} zwischen <strong><u>${svc1?.title || "Dienst"}</u></strong> (${getDayName(s1.date)}, ${s1.start_time}-${s1.end_time} Uhr) und <strong><u>${svc2?.title || "Dienst"}</u></strong> (${getDayName(s2.date)}, ${s2.start_time}-${s2.end_time} Uhr)`,
              userId,
  ...
  for (const s of shifts) {
    ...
    if (assignedCount > svc.max_persons) {
      conflicts.push({
        ...
        type: "overcapacity",
        message: `Überbelegung für <strong><u>${svc.title}</u></strong> am ${getDayName(s.date)}, ${formatDateGerman(s.date)} (${getDayName(s.date)}, ${s.start_time}-${s.end_time} Uhr): ${assignedCount} von max. ${svc.max_persons} Personen zugeordnet.`,
        shiftId1: s.id,
      });
    }
  }
